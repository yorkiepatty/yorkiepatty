...
:: Figure out where this installer lives
set "SOURCE_DIR=%~dp0"
...
:: Copy royce/ package to install directory
if exist "%SOURCE_DIR%royce\" (
    xcopy "%SOURCE_DIR%royce\*.*" "%INSTALL_DIR%\royce\" /E /I /Y /Q >nul 2>&1
    echo         Files copied from royce\ folder
) else (
    :: Fallback: files are in same dir as installer
    xcopy "%SOURCE_DIR%*.py" "%INSTALL_DIR%\royce\" /E /I /Y /Q >nul 2>&1
    xcopy "%SOURCE_DIR%*.json" "%INSTALL_DIR%\royce\" /E /I /Y /Q >nul 2>&1
    xcopy "%SOURCE_DIR%*.txt" "%INSTALL_DIR%\royce\" /E /I /Y /Q >nul 2>&1
    xcopy "%SOURCE_DIR%.env.example" "%INSTALL_DIR%\royce\" /I /Y /Q >nul 2>&1
    echo         Files copied
)
