"""
Memory Mesh Bridge - Adapter for Sonny Ultimate Voice
Allows seamless integration of MemoryMesh with existing Sonny code
"""
...
class MemoryMeshBridge:
    """
    Bridge adapter to make MemoryMesh compatible with existing Sonny code
    Provides the same interface as old MemoryManager but with MemoryMesh power
    """
    
    def __init__(self, memory_dir: str = "./sonny_memory", encryption_key: Optional[bytes] = None):
        """
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        """Get statistics about Sonny's memory"""
        if not hasattr(self, 'mesh') or self.mesh is None:
            logger.error("MemoryMesh not initialized")
            return {}
        try:
