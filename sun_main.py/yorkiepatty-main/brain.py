...
def boot():
    ...
    print("🚀 Sonny boot sequence continuing...")
...
try:
    from sonny_learning_coordinator import sonny_coordinator, start_sonny_learning
except ImportError:
    logger.warning("sonny_learning_coordinator not found, using fallback")

    class DummyCoordinator:
        def start(self):
            logger.info("Learning coordinator fallback active")

    sonny_coordinator = DummyCoordinator()

    def start_sonny_learning():
        sonny_coordinator.start()
...
class Sonny:
    def __init__(self, file_path: str = "./memory/memory_store.json"):
        ...
        self.learning_coordinator = sonny_coordinator

        logger.info(f"Sonny initialized successfully with memory file: {file_path}")

    def generate_greeting(self) -> str:
        """
        Returns a startup greeting when Sonny Dashboard launches.
        Sharp, confident, ready for business.
        """
        return "Sonny online. Ready to build something extraordinary. What are we working on?"
    ...
    def start_learning(self):
        """Activate Sonny's coordinated learning systems."""
        try:
            start_sonny_learning()
        except Exception as exc:  # pragma: no cover - defensive
            logger.error("Failed to start learning systems: %s", exc)
        else:
            logger.info("Sonny is now learning autonomously")
    ...
    def think(self, input_text: str):
        # Step 1: Check for Teaching/Learning Context
        learning_keywords = [
            "learn", "teach", "show me", "how do", "explain", "what is",
            "help with", "stuck", "error", "practice", "code", "program"
        ]
        is_learning_context = any(kw in input_text.lower() for kw in learning_keywords)
        is_yorkie = "yorkie" in input_text.lower()
        
        # Step 2: Detect Intent (with teaching priority)
        intent = detect_intent(input_text)
        
        if is_learning_context or is_yorkie:
            # Priority: Teaching and mentoring responses
            logger.info("Learning context detected - using teaching mode")
            try:
                # Use teaching assistant for specialized educational responses
                from teaching_assistant import get_teaching_response
                teaching_response, teaching_intent = get_teaching_response(input_text)
                if teaching_response:
                    return {
                        "intent": teaching_intent or intent,
                        "context": "Teaching Mode",
                        "response": teaching_response,
                        "mood": {"encouraging": True, "patient": True},
                        "teaching_focused": True
                    }
            except ImportError:
                logger.info("Teaching assistant not available, using regular response")

        # Step 3: Smarter question detection
        question_keywords = [
            "who is",
            "what is", 
            "what's",
    ...
    def run_self_repair(self, user_input, sonny_output):
...
# -------------------------------------------------------------
# Global Sonny instance (fixed)
# -------------------------------------------------------------
sonny = Sonny(file_path="./memory/memory_store.json")
