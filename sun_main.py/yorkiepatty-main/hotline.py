...
app = FastAPI(title="Sonny Dashboard")
        ...
        while True:
            ...
            try:
                ...
                if command == "tts":
                    ...
                    response = await sonny_chat_response(message)
                    await manager.send_personal_message(
                ...
                # Plain text message
                response = await sonny_chat_response(data)
...
async def generate_tts_response(text: str) -> Dict:
    ...
    try:
        audio_data = sonny_instance.speak(text)
        return {
            "success": True,
            "audio": audio_data,
            "text": text
...
async def sonny_chat_response(message: str) -> str:
    """Get Sonny's chat response"""
    try:
