...
try:
    from teaching_assistant import get_teaching_response, get_encouragement
    from learning_progress_tracker import progress_tracker
    TEACHING_MODE = True
    logger.info("Teaching assistant capabilities loaded")
except ImportError:
    TEACHING_MODE = False
    logger.info("Teaching mode disabled - modules not found")
...
class ConversationEngine:
    ...
    def process_text(
        ...
        # Check for teaching opportunities first (Sonny's priority mode)
        if TEACHING_MODE:
            teaching_response, teaching_intent = get_teaching_response(cleaned_text)
            if teaching_response:  # Teaching system found a relevant response
                logger.info(f"Using teaching response for intent: {teaching_intent}")
                # Override regular response with teaching-focused one
                response_text = teaching_response
                emotion = "encouraging"
                emotion_tier = 1
                
                # Track learning interaction
                if user_id == "Yorkie" or "yorkie" in cleaned_text.lower():
                    # This could be a learning session
                    progress_tracker.log_question_asked("current_session", cleaned_text)
            else:
