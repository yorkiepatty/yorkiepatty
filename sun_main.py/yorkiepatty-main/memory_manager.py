...
class MemoryManager:
    """Manager for Sonny's long-term memory storage with persistent GitHub backup"""
    ...
    def load(self):
        ...
        print("🧠 Loading Sonny's persistent memory...")
    ...
    def _backup_to_github(self):
        ...
        try:
            ...
            commit_msg = f"💾 Auto-backup: Sonny's memory update ({datetime.now().strftime('%Y-%m-%d %H:%M')})"
            subprocess.run(
