#!/usr/bin/env python3
"""
Test Memory Mesh Integration with Sonny
...
sys.path.insert(0, '/workspaces/SonnyC')
...
def test_memory_mesh_core():
    ...
    # Initialize
    memory = MemoryMesh(memory_dir="./test_sonny_memory")
    ...
    # Verify files exist
    memory_dir = Path("./test_sonny_memory")
    ...
    memory2 = MemoryMesh(memory_dir="./test_sonny_memory")
...
def test_memory_mesh_bridge():
    """Test Memory Mesh Bridge (Sonny compatibility layer)"""
    print("=" * 70)
    print("🧪 TEST 2: Memory Mesh Bridge (Sonny Compatibility)")
    print("=" * 70)
    
    # Initialize bridge
    bridge = MemoryMeshBridge(memory_dir="./test_sonny_memory_bridge")
    
    # Test 1: Store with old interface
    print("\n1️⃣  Testing old MemoryManager interface...")
    bridge.store("user_name", "Nathaniel")
    bridge.store("project_goal", "Sonny learns from master AIs")
    ...
    bridge.save()
    memory_dir = Path("./test_sonny_memory_bridge")
    ...
    bridge2 = MemoryMeshBridge(memory_dir="./test_sonny_memory_bridge")
    bridge2.load()
...
def test_sonny_integration_scenario():
    """Test realistic Sonny conversation scenario"""
    print("=" * 70)
    print("🧪 TEST 4: Realistic Sonny Conversation Scenario")
    print("=" * 70)
    
    # Initialize Sonny's memory
    sonny_memory = MemoryMeshBridge(memory_dir="./test_sonny_conversation")
    
    # Simulate conversation sequence
    print("\n📞 Simulating Sonny conversation...\n")
    
    # User introduces themselves
    print("👤 User: Hi Sonny, I'm Nathaniel, the creator of this project")
    sonny_memory.store(
        "user_introduction",
    ...
    # Sonny responds and remembers
    print("🤖 Sonny: Nice to meet you, Nathaniel!")
    sonny_memory.store(
        "sonny_greeting",
    ...
    context = sonny_memory.retrieve_relevant("AlphaVox")
    print(f"   🧠 Sonny recalls: {context if context else 'No prior knowledge'}")
    ...
    # Later conversation - Sonny should remember
    print("\n👤 User: What's my name?")
    context = sonny_memory.retrieve_relevant("name creator Nathaniel")
    print(f"   🧠 Sonny recalls: {context[:100]}...")
    assert "Nathaniel" in context, "Sonny should remember user's name"
    print("🤖 Sonny: Your name is Nathaniel!")
    
    # User asks about past conversation
    print("\n👤 User: What did we talk about earlier?")
    recent = sonny_memory.get_recent_context(hours=1)
    print(f"   🧠 Sonny recalls recent context: {len(recent)} chars")
    print("🤖 Sonny: We discussed your introduction and AlphaVox")
    
    # Check memory stats
    print("\n📊 Sonny's Memory Status:")
    stats = sonny_memory.get_memory_stats()
    for key, value in stats.items():
        if not isinstance(value, dict) and not isinstance(value, list):
            print(f"   {key}: {value}")
    
    print("\n✅ TEST 4 PASSED: Sonny can maintain conversation context!\n")
    return True
...
def cleanup_test_directories():
    ...
    test_dirs = [
        "./test_sonny_memory",
        "./test_sonny_memory_bridge",
        "./test_human_memory",
        "./test_sonny_conversation"
    ]
...
def main():
    ...
    try:
        ...
        test4 = test_sonny_integration_scenario()
        ...
        print(f"✅ Sonny Integration: {'PASSED' if test4 else 'FAILED'}")
        print()
        
        if all([test1, test2, test3, test4]):
            print("🎉 ALL TESTS PASSED! Memory Mesh is production-ready!")
            print()
            print("✅ Sonny now has human-like memory:")
            ...
            print()
            print("🚀 Sonny is ready to remember, learn, and grow!")
            return 0
        else:
