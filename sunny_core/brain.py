...
def boot():
    ...
    print("🚀 Sunny boot sequence continuing...")
...
try:
    from sunny_learning_coordinator import derek_coordinator, start_derek_learning
except ImportError:
    logger.warning("sunny_learning_coordinator not found, using fallback")
...
class Sunny:
    def __init__(self, file_path: str = "./memory/memory_store.json"):
        ...
        logger.info(f"Sunny initialized successfully with memory file: {file_path}")

    def generate_greeting(self) -> str:
        """
        Returns a startup greeting when Sunny Dashboard launches.
        Can be made dynamic later, but static is fine to unblock startup.
        """
        return "Hello, I’m Sunny — ready to assist you."
    ...
    def start_learning(self):
        """Activate Sunny's coordinated learning systems."""
        try:
            start_derek_learning()
        except Exception as exc:  # pragma: no cover - defensive
            logger.error("Failed to start learning systems: %s", exc)
        else:
            logger.info("Sunny is now learning autonomously")
...
# -------------------------------------------------------------
# Global Sunny instance (fixed)
# -------------------------------------------------------------
sunny = Sunny(file_path="./memory/memory_store.json")
