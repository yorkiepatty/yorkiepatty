...
--------------------
Handles Sunny's secure connection to online knowledge sources.
...
def main():
    ...
    while True:
        query = input("🔎 Ask Sunny something (or 'exit'): ").strip()
        if query.lower() in ("exit", "quit"):
            print("👋 Exiting Internet Mode.")
            break

        result = query_internet(query)
        print(f"\n🧠 Sunny (Internet): {result.get('summary', 'No response')}\n")
