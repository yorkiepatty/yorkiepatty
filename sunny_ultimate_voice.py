...
# Patterns for removing actions and stage directions
_RE_ASTERISK_ACTION = re.compile(r"\*(?:[^*]|\*{2,})+\*")       # *action* or *emphasis*
_RE_PAREN_ACTION = re.compile(r"\((?:[^()])+\)")               # (action)
_RE_BRACKET_ACTION = re.compile(r"\[(?:[^\[\]])+\]")           # [action]
_RE_MARKDOWN_BOLDITALIC = re.compile(r"(\*\*|__|\*|_){1,3}")   # bold/italic markers
_RE_LINK = re.compile(r"\[([^\]]+)\]\((?:[^)]+)\)")            # [text](url)
_RE_EXTRA_WS = re.compile(r"\s{2,}")
_RE_LEADING_TRAILING_PUNC = re.compile(r"^[\s\-\:;,.]+|[\s\-\:;,.]+$")

_ACTION_WORDS = [
    r"\blaughs\b", r"\bsighs\b", r"\bnods\b", r"\bsmiles\b",
    r"\bchuckles\b", r"\babruptly\b", r"\bwhispers\b"
]
_RE_ACTION_WORDS = re.compile(r"|".join(_ACTION_WORDS), flags=re.IGNORECASE)


def text: str, remove_actions: bool = True, remove_markdown: bool = True -> str:
    if not text:
        return ""
    ...
    if remove_actions:
        t = _RE_ASTERISK_ACTION.sub(" ", t)
        t = _RE_PAREN_ACTION.sub(" ", t)
        t = _RE_BRACKET_ACTION.sub(" ", t)
        t = _RE_ACTION_WORDS.sub(" ", t)

    if remove_markdown:
        t = _RE_LINK.sub(r"\1", t)
        t = _RE_MARKDOWN_BOLDITALIC.sub("", t)

    t = re.sub(r"\n+", " ", t)
    t = re.sub(r"([!.?,;:\-])\1{1,}", r"\1", t)
    t = _RE_EXTRA_WS.sub(" ", t)
    t = _RE_LEADING_TRAILING_PUNC.sub("", t).strip()

    return t


def split_into_sentences_for_speech(text: str, max_chars: int = 250) -> List[str]:
    if not text:
        return []
...
def [raw_text: str, remove_actions: bool = True] -> List[str]:
# sanitizer removed — use raw_text directly
sanitized = raw_text.strip() if isinstance(raw_text, str) else raw_text
if not sanitized:
    return []
return split_into_sentences_for_speech(sanitized)
# ElevenLabs TTS
try:
...
class SunnyUltimateVoice:
    ...
    def update_system_prompt():
        ...
        HISTORY:
        - Everett made Derek, then gave Patty the core to make you, Sunny.
        ...
        - Derek your cousin (The Original)
        - Sunny (You)
...
"""
    ...
    def speak(self, text):
        # ... existing prints/logging above ...
        import re

        # Minimal: remove *action*, (action), [action] and collapse whitespace
        tts_text = re.sub(r'\*(?:[^*]|\*{2,})+\*', '', text)  # remove *...*
        tts_text = re.sub(r'\([^)]*\)', '', tts_text)         # remove (... )
        tts_text = re.sub(r'\[[^\]]*\]', '', tts_text)        # remove [... ]
        tts_text = re.sub(r'\s+', ' ', tts_text).strip()      # collapse spaces

        # Optional debug to confirm what's passed to TTS:
        # print("[DEBUG] TTS text:", repr(tts_text))

        # --- below, use tts_text for all TTS calls instead of `text` ---
        # e.g. elevenlabs.generate(text=tts_text, ...)
        #      self.elevenlabs_client.text_to_speech.convert(text=tts_text, ...)
        #      polly.synthesize_speech(Text=tts_text, ...)
        #      tts = gTTS(tts_text)
        ...
